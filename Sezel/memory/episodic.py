...
class EpisodicStore:
    ...
    async def unconsolidated(self, n: int = 10) -> list[Turn]:
        """Get turns that haven't been consolidated into semantic memory yet.
        Returns:
                    List of Turn objects that need consolidation
        """
    ...
    async def mark_consolidated(self, turns: list[Turn]) -> None:
        """
        Mark turns as consolidated (salience = 1.0).
        "These have been processed already, don't look at them again."

        Args:
            turns: List of turns to mark as consolidated
        """
        ...
        def _mark():
            for turn in turns:
                conn.execute(
                    "UPDATE episodes SET salience = 1.0 WHERE ts = ? AND role = ? AND content = ?",
                    (turn.ts, turn.role, turn.content)
                )
            conn.commit()
